...
def slicer(x, index):
	return x[:, tensorflow.convert_to_tensor(index, dtype=tensorflow.int32), :]
...
@keras.utils.register_keras_serializable()
def flattener(x):
	x = tensorflow.reshape(x, [-1, x.shape[1]*x.shape[3]])
	return x
...
@keras.utils.register_keras_serializable()
def stack(x):
	return tensorflow.stack(x, axis=1)

class bc_LSTM:

	def __init__(self, args):
		self.classification_mode = args.classify	# sınıflandırma modu [Sentiment|Emotion]
		self.modality = args.modality	# modalite türü [text|audio|bimodal]
		self.PATH = "./data/models/{}_weights_{}.keras".format(args.modality,self.classification_mode.lower())	# model ağırlıklarının kaydedileceği path oluşturulur
		self.OUTPUT_PATH = "./data/pickles/{}_{}.pkl".format(args.modality,self.classification_mode.lower())	# ara çıktılar için pickle dosyasının pathi oluşturulur
	...
	def load_data(self,):
		...
		self.train_mask = self.data.train_mask  # Masklar (gereksiz pad edilen verileri dışlamak için)
	...
	def get_audio_model(self):	# ses için model tanımlanır
		...
		lstm = Bidirectional(LSTM(300, activation='tanh', return_sequences = True, dropout=0.4))(masked)	# iki yönlü lstm
	...
	def get_text_model(self):
		...
		def reshaper(x):
			return tensorflow.expand_dims(x, axis=3)
		...
		for ind in range(self.sequence_length):
			...
			reshape = Lambda(lambda x: tensorflow.expand_dims(x, axis=-1), output_shape=(50, 300, 1))(emb_output)
		...
		def stack(x):
			return tensorflow.stack(x, axis=1)
		cnn_outputs = Lambda(stack, output_shape=(None, 100))(cnn_output)
	...
	def train_model(self):
		...
		early_stopping = EarlyStopping(monitor='val_loss', patience=10)	# modelin aşırı önlemesini önlemek için
	...
	def test_model(self):
		print("RABİAAAAAAAAAAAAAA--------------------")
		print(self.PATH)
		model = load_model(self.PATH, custom_objects={"slicer": slicer})
		intermediate_layer_model = Model(inputs=model.input, outputs=model.get_layer("utter").output)	# modelin ara katman çıktıları (utter katmanı) alınıp kaydediliyor
		...
		pickle.dump([train_emb, val_emb, test_emb], open(self.OUTPUT_PATH, "wb"))	# ara katman özellikleri kaydediliyor
